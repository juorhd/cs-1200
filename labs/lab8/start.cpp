...
void calc_path( int x, int y, bool &startpoint, vector< vector<int> > &sol, 
						const vector< vector<bool> > &blocked_grid){
	if ( startpoint == true ){
		startpoint = false;
		calc_path(x, y-1, startpoint, sol, blocked_grid);
		calc_path(x-1, y, startpoint, sol, blocked_grid);
		return;
	}
	if ( x < 0 || y < 0 || blocked_grid[x][y] ) return;
	if ( x+1 == blocked_grid.size() ){
		sol[x][y] = sol[x][y+1];
	} 
	else if ( y+1 == blocked_grid[0].size() ){
		sol[x][y] = sol[x+1][y];
	} 
	else sol[x][y] = sol[x][y+1] + sol[x+1][y];
	calc_path(x, y-1, startpoint, sol, blocked_grid);
	calc_path(x-1, y, startpoint, sol, blocked_grid);
}

void find_path( int start_x, int start_y, int x, int y, const vector< vector<int> > &pathNum, 
								vector<Point> &path, bool &found){
	...
	if ( found ) return;
	if( x-1 >= 0 && pathNum[x-1][y] > 0 && pathNum[x-1][y] <= pathNum[0][0] ){
		find_path(start_x, start_y, x-1, y, pathNum, path, found);
		path.pop_back();
	}
	if ( y-1 >= 0 && pathNum[x][y-1] > 0 && pathNum[x][y-1] <= pathNum[0][0] ){
		find_path(start_x, start_y,x, y-1, pathNum, path, found);
		path.pop_back();
	}
...
int main(int argc, char* argv[]) {
	...
	vector<vector<int> > pathNum(blocked_grid.size(), vector<int> (blocked_grid[0].size(), 0));
	pathNum[start_x][start_y] = 1;
	...
	calc_path(start_x, start_y, startpoint, pathNum, blocked_grid);
	cout << endl;
	for (int y=0; y<blocked_grid[0].size(); ++y) {
		for (int x=0; x<blocked_grid.size(); ++x) {
			if ( pathNum[x][y] < 10 && pathNum[x][y] <= pathNum[0][0] ) cout << "   ";
			else if ( pathNum[x][y] < 100 && pathNum[x][y] <= pathNum[0][0]) cout << "  ";
			else cout <<' ';
			if (pathNum[x][y] > pathNum[0][0] ) cout << "BIG";
			else cout << pathNum[x][y];
		}
		cout << endl;
	}
	vector<Point> path;
	bool found = false;
	find_path(start_x, start_y, start_x, start_y, pathNum, path, found);
