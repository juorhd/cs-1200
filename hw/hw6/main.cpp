...
// Helper function to check if every unknown ships is in order
bool everyShipLen ( const vector<char> &ships, int start_index,
				            int maxshiplength ){
	char maxlen = min( maxshiplength, 7 ) + '0';
	for ( int i = start_index; i < ships.size(); ++i ){
		if ( ships[i] > maxlen ) return false;
	}
	return true;
}
...
void UnknownHandler( vector<char> &ships, vector< vector<char> > &ships_possibilities, 
	...
	for ( int i = new_start; i < ships.size(); ++i ) {
		ships[i]++;
		if ( everyShipLen( ships, start_index, maxshiplength ) 
						&& orders( ships, start_index) ) {
								
			UnknownHandler(ships, ships_possibilities, start_index, i,
					       minlength, maxlength, maxshiplength);			
		}	
		ships[i]--;	
	}
}
...
int main( int argc, char *argv[] ) {
	...
	if ( all_solution ) {
		...
		else cout << "find " << count << " solution(s)\n";
	}
	else if ( count == 0 ) cout << "No Solution.\n";
