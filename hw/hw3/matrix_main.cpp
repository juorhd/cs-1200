...
void StudentTest(){
	...
	m9.set(2,2,-1);
	m9.set(2,5,1);
	std::cout << "Original:" << m9 << std::endl;
	m9.transpose();
	std::cout << "afterTranspose:" << m9 << std::endl;
	
	// ------------------------------------------------
	// multiply_by_coefficient test
	//	2 2 2  x4.5   9 9 9
	//	2 2 2 ======> 9 9 9
	//	2 2 2	      9 9 9
	
	// set metrix and do multiply
	Matrix t1(3,3,2);
	t1.multiply_by_coefficient(4.5);
	...
	//	9 9 9	get the first column   9
	//	9 9 9 =======================> 9
	//	9 9 9	 		       9

	// using t1, and get_col
	double* d2 = NULL;
	d2 = t1.get_col(0);
	...
	// 	9 9 9      -9.5	    -0.5 -0.5 -0.5
	// 	9 9 9     ======>   -0.5 -0.5 -0.5 
	// 	9 9 9 		    -0.5 -0.5 -0.5

	// set metrix
	Matrix t2(4, 4, 5);
	assert(t2.subtract(t1) == false); // not work
		
	// set another metrix and do subtract
	Matrix t3(3, 3, 9.5);
	t1.subtract(t3); // t1 - t3
	...
	// ------------------------------------------------
	// test for copy of empty matrix

	Matrix t6;
	Matrix t7(t6);
	assert(t7.num_rows() == 0 && t7.num_cols() == 0);
	
	// ------------------------------------------------
	// test for empty matrix addition/multiplication/funcs
	...
	t8.multiply_by_coefficient(5);
	assert ( t8 == t9 );
	
	double* g_col = t8.get_col(0);
	double* g_row = t8.get_row(0);
	assert ( g_col == NULL && g_row == NULL );
}
